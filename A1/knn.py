...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in range(num_train):
    dists[i,:] = torch.sum((x_train[i]-x_test)**2,dim=1)**0.5
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  ##############################################################################
  # Replace "pass" stax_train**2,dim=1tement with your code
  x_train=x_train.reshape(num_train,-1)
  x_test=x_test.reshape(num_test,-1)
  x_train_square = torch.sum(x_train**2,dim=1)
  x_test_square = torch.sum(x_test**2,dim=1)
  x_multiply = torch.mm(x_train,x_test.t())
  dists = (x_train_square.view(-1,1) + x_test_square - 2 * x_multiply) ** 0.5
  # 将形状的第二个参数调整为1才能过测试
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  _, indices = torch.topk(dists,largest = False,dim = 0,k = k)
  index = y_train[indices]
  y_pred, _ = torch.mode(index,dim = 0)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
      ...
      knn_classifier = KnnClassifier(x_train_data,y_train_data)
      acc = knn_classifier.check_accuracy(x_test_data,y_test_data,k = k,quiet = True)
