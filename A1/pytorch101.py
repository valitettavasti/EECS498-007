...
def mutate_tensor(x, indices, values):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  for index,indices in enumerate(indices):
    x[indices[0],indices[1]] = values[index]
  ...
  if use_loop:
    for i in range(B):
      ...
      for j in range(N):
        for k in range(P):
          for l in range(M):
            z[i,j,k] = z[i,j,k]+x[i,j,l]*y[i,l,k]
      """
  else:
    z = torch.bmm(x,y)
  #############################################################################
...
def normalize_columns(x):
  ...
  u = torch.sum(x,dim=0)/x.shape[0]
  x1 = torch.square(x-u)
  x2 = torch.sum(x1,dim = 0)
  sigma = torch.sqrt(x2/(x.shape[0]-1))
  y = (x-u)/sigma
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
