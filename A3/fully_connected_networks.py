...
class ReLU(object):
  ...
  def forward(x):
    ...
    out = torch.maximum(zero_tensor, x)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = x
...
class TwoLayerNet(object):
  ...
  def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
    ...
    mean = 0
    std = weight_scale
    W1 = torch.normal(mean=mean, std=std, size=(input_dim, hidden_dim), device = device, dtype = dtype)
    W2 = torch.normal(mean=mean, std=std, size=(hidden_dim, num_classes), device = device, dtype = dtype)
    self.params['W1'] = W1
    self.params['W2'] = W2
  ...
  def loss(self, X, y=None):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    # scores, cache2 = ReLU.forward(scores)
    ...
    grad_1, dw2, db2 = Linear.backward(grad, cache2)
    dw2 += 2 * self.reg * self.params['W2']
    _, dw1, db1 = Linear_ReLU.backward(grad_1, cache1)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    mean = 0
    std = weight_scale
    W1 = torch.normal(mean=mean, std=std, size=(input_dim, hidden_dims[0]), device=device, dtype=dtype)
    b1 = torch.zeros(hidden_dims[0], device=device, dtype=dtype)
    self.params['W1'] = W1
    self.params['b1'] = b1
    for i in range(len(hidden_dims) - 1):
      W = torch.normal(mean=mean, std=std, size=(hidden_dims[i], hidden_dims[i+1]), device=device, dtype=dtype)
      b = torch.zeros(hidden_dims[i+1], device=device, dtype=dtype)
      self.params[f'W{i + 2}'] = W
      self.params[f'b{i + 2}'] = b
    W = torch.normal(mean=mean, std=std, size=(hidden_dims[-1], num_classes), device=device, dtype=dtype)
  ...
  def loss(self, X, y=None):
    ...
    caches = []
    dropout_caches = []
    score, cache = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
    caches.append(cache)
    if self.use_dropout:
      score, cache = Dropout.forward(score, self.dropout_param)
      dropout_caches.append(cache)
    scores_list.append(score)
    for i in range(2,self.num_layers):
      score, cache = Linear_ReLU.forward(scores_list[-1], self.params[f'W{i}'], self.params[f'b{i}'])
      caches.append(cache)
      if self.use_dropout:
        score, cache = Dropout.forward(score, self.dropout_param)
        dropout_caches.append(cache)
      scores_list.append(score)
    score, cache = Linear.forward(scores_list[-1], self.params[f'W{self.num_layers}'],
    ...
    ############################################################################
    # Replace "pass" statement with your code
    loss, grad = softmax_loss(scores_list[-1], y)
    for i in range(self.num_layers):
      loss += self.reg * torch.sum(self.params[f'W{i+1}'] * self.params[f'W{i+1}'])
    grad_1, dw, db = Linear.backward(grad, caches[-1])
    dw += self.reg * 2 * self.params[f'W{self.num_layers}']
    db += self.reg * 2 * self.params[f'b{self.num_layers}']
    grads[f'W{self.num_layers}'] = dw
    grads[f'b{self.num_layers}'] = db
    for i in range(2, self.num_layers+1):
      if self.use_dropout:
        grad_1 = Dropout.backward(grad_1, dropout_caches[-i+1])
      grad_1, dw, db = Linear_ReLU.backward(grad_1,caches[-i])
      dw += self.reg * 2 * self.params[f'W{self.num_layers-i+1}']
      db += self.reg * 2 * self.params[f'b{self.num_layers-i+1}']
      grads[f'W{self.num_layers-i+1}'] = dw
      grads[f'b{self.num_layers-i+1}'] = db
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
