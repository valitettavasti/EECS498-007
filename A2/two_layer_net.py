...
def nn_forward_pass(params, X):
    ...
    zero_tensor = torch.zeros_like(hidden)
    hidden = torch.maximum(hidden, zero_tensor)
...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    # Replace "pass" statement with your code
    # scores:(N, C) y:(N,)
    scores -= torch.max(scores, dim=1, keepdim=True).values # 数值稳定,免得exp()溢出导致NaN
    probabilities = torch.exp(scores)  # (N, C)
    prob_sum = torch.sum(probabilities,dim = 1).reshape(-1,1)
    probabilities /= prob_sum
    # probabilities = torch.softmax(scores,dim=1)
    loss = 0.0
    loss += torch.sum(-torch.log(probabilities[range(N), y]))
    loss /= N
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    grad = probabilities # (N,C)
    grad[range(N), y] -= 1
    grad /= N
    grad_W2 = h1.t().mm(grad) + 2 * reg * W2 #(H,C)
    grad_h1 = grad.mm(W2.t())  #(N,H)
    grad_b2 = torch.sum(grad, dim = 0)
    mask = torch.zeros_like(h1)     
    mask[h1 > 0] = 1
    grad_relu = mask * grad_h1  #(N,H)
    #grad_relu = grad_h1
    #grad_relu[h1<=0] = 0
    grad_W1 = X.t().mm(grad_relu) + 2 * reg * W1 # (D,H)
    grad_b1 = torch.sum(grad_relu, dim = 0)
    grads['W2'] = grad_W2
    grads['b2'] = grad_b2
    grads['W1'] = grad_W1
    grads['b1'] = grad_b1
...
def nn_predict(params, loss_func, X):
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  scroes=loss_func(params,X)  #(N,C)
  y_pred = torch.argmax(scroes,dim = 1)
  ###########################################################################
  #                              END OF YOUR CODE                           #
  ###########################################################################
...
def nn_get_search_params():
  ...
  regularization_strengths = [0.0008, 0.001]
    ...
    for hidden_size in hidden_sizes:
      for regularization_strength in regularization_strengths:
        for learning_rate_decay in learning_rate_decays:
          ...
          print("Stat:" + " learning_rate:" + str(learning_rate) + \
          " hidden_size:" + str(hidden_size) + \
          " regularization_strength:" + str(regularization_strength) + \
          " learning_rate_decay:" + str(learning_rate_decay) + \
          " loss:" + str(stat['loss_history'][-1]) + \
          " train_acc:" + str(stat['train_acc_history'][-1]) + \
          " val_acc:" + str(stat['val_acc_history'][-1])
          )
          if stat['val_acc_history'][-1] > best_val_acc:
            best_net = net
            best_stat = stat
            best_val_acc = stat['val_acc_history'][-1]
            best_train_acc = stat['train_acc_history'][-1]
            best_loss = stat['loss_history'][-1]
            best_parameter = [learning_rate,hidden_size,regularization_strength,learning_rate_decay]
  print("best stat:" + "learning_rate:" + str(best_parameter[0]) + \
      " hidden_size:" + str(best_parameter[1]) + \
      " regularization_strength:" + str(best_parameter[2]) + \
      " learning_rate_decay:" + str(best_parameter[3]) + \
      " loss:" + str(best_loss) + \
      " best_train_acc:" + str(best_train_acc) + \
