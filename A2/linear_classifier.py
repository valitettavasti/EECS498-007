...
def svm_loss_vectorized(W, X, y, reg):
  ...
  correct_class_score = scores[range(N), y].reshape(-1, 1) # (N,)
  zeros = torch.zeros_like(scores) # (C,)
  margin = torch.maximum(scores - correct_class_score + 1, zeros)
  margin[range(N), y] = 0
...
def predict_linear_classifier(W, X):
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  outcome_tensor = X.mm(W)    # (N,C)
  y_pred = outcome_tensor.argmax(dim=1)
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
  return y_pred
...
def svm_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [0.005, 0.008, 0.01, 0.012]
  regularization_strengths = [0.0005, 0.0008, 0.001, 0.005]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  val_acc = 100 * (data_dict['y_val'] == y_val_pred).double().mean().item()
  ############################################################################
  #                            END OF YOUR CODE                              #
  ############################################################################
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    ...
    loss += -torch.log(probabilities[y[i]])
    for j in range(num_classes):
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  loss += torch.sum(-torch.log(probabilities[range(num_train), y]))
  loss /= num_train
  loss += reg * torch.sum(W * W)
  probabilities[range(num_train), y] -= 1
  dW = X.t().mm(probabilities)
  dW /= num_train
  dW += reg * 2 * W
  #############################################################################
  #                          END OF YOUR CODE                                 #
